...
<body>
	<div id="container">
		...
		<%
			List<notify> listCon = (ArrayList) request.getAttribute("listCon");
		%>
		<div id="body" class="width">
			...
			<br><form action="${pageContext.request.contextPath}/Search?notify=notify" method="POST">
                                <input type="text" name="ten" id="searchaccout" placeholder="Search for name content">
                                <input type="submit" value="Search" class="login" />        
                            </form>
			<p style="text-align: center;font-size: 25px;color: red">Thông Báo</p>
			<a href="${pageContext.request.contextPath}/ListContent?view_all=1"><p style="text-align: right;font-size: 18px;">Xem các bài đăng</p></a>
			<table id="bootstrap-table"
				...
				<tbody>
				<%
					for (notify notify : listCon) {
				%>
					<tr>
						<td><h2><%=notify.getTitle()%></h2></td>
					</tr>
					
					<tr>
						<td><div class="article-info">
								Tác giả :<%=notify.getAuthor()%></div> <br>
							<p><%=notify.getContent()%></p> 
							<br>
							<%
							String f = notify.getFile();
							if(f != null){
								%>
							...
							<a style="text-decoration: none; float: right;"><%=notify.getDate()%></a></td>
					</tr>
					<%
					}
				%>
